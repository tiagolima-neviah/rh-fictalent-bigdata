...
| 1 · staging (réplica) | MySQL 8 (ADR-0001) com a DDL dos 10 módulos aplicada: 75 tabelas de negócio mais a trilha de exclusões, comentário em toda tabela, chaves entre databases, etiqueta LGPD por coluna trilha de exclusões por gatilho gerado da DDL e papéis por função com GRANT gerado das etiquetas LGPD, provados por teste, cifra em repouso de tudo com keyring próprio e dicionário de dados gerado da `information_schema` ([Modelo de Dados](04_modelo_dados_staging.md), v0.2.0, cards 2.2 a 2.7); calibração do dado sintético iniciada: índice sazonal por mês, escopo e grupo CNAE derivado dos microdados do Novo CAGED, com a fonte, em `dados/publicos/caged` (v0.4.0, card 4.1); a régua de validação existe antes do gerador ([ADR-0006](adr/0006-pandera-e-regua.md)): 165 checks em seis famílias (escala, naturalidade, a história, sazonalidade contra o CAGED, coerência interna e sujeira na medida certa), com laudo aprovado, reprovado ou incompleto e a regra "reprovou, regenera" (`python -m rh_fictalent.validacao --contrato`, card 4.3); o gerador determinístico começou pela etapa 1, o mundo cadastral: as 12 tabelas de `cadastro` (111 municípios da tabela do IBGE em 5 regiões, 3 filiais, 45 funções, convenções com piso por função e ano, feriados da BrasilAPI mais os locais, escalas, motivos, parâmetros), com ids do gerador, carimbos no instante histórico do fato e escrita na réplica como `replicador` (`python -m rh_fictalent.gerador --etapa 1 --gravar`, card 4.4); etapa 2, a carteira comercial simulada mês a mês de 2018-01 a 2026-09: clientes entram perseguindo a meta do comercial, renovam, saem por mercado na renovação, por redução na pandemia e por serviço meses depois de a qualidade sentida passar do limite de cada um (a defasagem de 6 a 9 meses nasce da mecânica, não de um parâmetro); postos abrem e fecham para sustentar o headcount alvo com o ritmo do ano; preço por posto com reajuste no aniversário; reclamações que crescem com a degradação. As 8 tabelas de `comercial` mais endereços e centros de custo de contrato, e a régua parcial aprovada em 22 de 22 checks (card 4.5); etapa 3, o funil e as pessoas: um motor de ocupação decide, posição a posição, quando a vaga abre, quanto demora, se fecha, se a pessoa aparece no primeiro dia, quanto fica e por que sai (o temporário é contratado por demanda e roda, o que explica milhares de vagas por ano), e isso vira as 9 tabelas de `ats` e as 8 de `pessoas`, 1,33 milhão de linhas, com a sujeira de origem do catálogo (candidato duplicado, CPF inválido, nascimento impossível, admissão retroativa, temporário além do prazo legal) e um gabarito que não vai para a réplica; régua parcial aprovada em 134 de 134 checks, com os parâmetros por ano achados por um calibrador (`python -m rh_fictalent.gerador.calibrar_ocupacao`, card 4.6); etapa 4, o ponto e a folha, a etapa de volume: cada dia programado pela escala do posto vira apontamento e, nos dias trabalhados, as quatro batidas do relógio (com a batida que falta e a que se repete, do catálogo de sujeira); o mês de cada contrato vira itens de folha, a provisão acumula e é baixada na rescisão e no 13º, e o rateio leva o custo de cada pessoa até o posto. São 6,83 milhões de linhas geradas em numpy em 18 s e gravadas em pouco mais de 3 minutos; a réplica passa de 8 milhões de linhas e 1,4 GB, e o custo da folha bate com o rateio em todas as competências (card 4.7); etapa 5, o financeiro: a medição do mês vira fatura e item por posto, as colocações de R&S viram honorário, cada fatura gera um título (pago em dia, com atraso, ou nunca: a inadimplência cresce na crise), a folha, os benefícios e os impostos do lucro presumido viram títulos a pagar, e a despesa da retaguarda fecha o resultado na margem líquida do caso, sempre positiva (4% a 12%) e com a retaguarda entre 5% e 17% da receita; o consolidado gerencial bate com a operação até 2021 e diverge de 0,5% a 9% de 2022 a 2026, na réplica e em nove planilhas Excel versionadas em [`dados/gerencial`](../dados/gerencial/README.md), que são a fonte de arquivo do pipeline (card 4.8); etapa 6, conformidade e acesso: os 40 usuários internos ao longo do arco, com perfil de acesso em vigência, a matriz de permissão por módulo e a trilha de auditoria (cada CRIAR e EDITAR aponta para um registro que existe, no instante do carimbo dele, feito por alguém do setor e da filial que estava na casa naquele dia; as exportações de planilha crescem com a degradação); treinamento por exigência da função, com certificado que vence e reciclagem que atrasa; o exame admissional de todo contrato de trabalho, o periódico, o de retorno e o demissional quando a norma não dispensa, os programas legais por contrato de cliente, os riscos do posto (de onde vem o adicional que a folha paga), os acidentes e a CAT. O ASO vencido com a pessoa alocada é conduzido como prevalência: no último dia de cada mês, a parte dos alocados sem ASO válido, perto de zero até 2022 e em 4% em 2024 e 2025. A etapa também preenche o entrevistador das entrevistas da etapa 3 (o usuário só passa a existir aqui): é o único retoque do gerador, feito na mesma transação e sem mexer no carimbo da linha (card 4.9). O aceite (`--aceite --replica`) gera as seis etapas de ponta a ponta, junta as medidas, conta as linhas na réplica e passa a régua inteira: 165 de 165, com as medidas e o laudo versionados em [`dados/regua`](../dados/regua/README.md) (card 4.10). A ingestão relacional começou: o backfill traz a réplica inteira para a bronze em parquet (75 assets, um por tabela, particionados pelo ano de `criado_em`), lendo em lote com cursor de streaming e conferindo a contagem dentro de uma foto consistente do InnoDB; 8,4 milhões de linhas em 162 MB e 124 segundos, sem divergência em nenhuma das 675 partições (card 5.1). A carga incremental pede à réplica só o que tem `atualizado_em` acima da marca d'água, descobre em que partições as linhas caem e reescreve por merge de id, conferindo a contagem de cada partição tocada; a marca mora no warehouse, é o relógio da réplica, e a carga volta uma hora no tempo para não perder transação que commita depois do corte ([ADR-0011](adr/0011-marca-dagua-nao-binlog.md)). Agenda diária às 5h, a primeira do projeto; carga sem nada a fazer em 845 ms (card 5.2). As exclusões entram pela trilha que os gatilhos alimentam: a linha apagada na réplica **não some da bronze**, ganha `excluido_em`, e a conferência passa a contar só as vivas; a própria trilha é copiada para a bronze, porque recopiar uma tabela traz o presente e esqueceria o que morreu (card 5.3). A terceira natureza de fonte entrou: as nove planilhas do consolidado gerencial são lidas com esquema **pandera** declarado (237 linhas), depois de descartar o título e a linha de TOTAL e de consertar os tipos que a planilha misturou; esquema reprovado, asset reprovado, e a carga para no arquivo em vez de levar número errado três camadas adiante (card 5.4). Por fim, a réplica passou a se mexer sozinha: um job escreve o dia de operação que falta (ponto de quem está em campo, vagas fechando, alocações terminando e, de vez em quando, uma batida apagada), a carga incremental o encontra pela marca d'água e a trilha explica o que sumiu. A agenda dele nasce **desligada**, porque ligar faz a réplica deixar de ser exatamente a base que a régua aprovou; o que a operação acrescenta fica registrado em `ingestao.dia_simulado`, para a conferência continuar possível (card 5.5) |
...
| 4 · lake | a bronze é lida como um banco: uma conexão DuckDB em memória com **79 views com os nomes da réplica** (as 75 tabelas de negócio, a trilha `meta.exclusao_auditoria`, a planilha `arquivo.consolidado_gerencial` e as fontes `fontes.municipios` e `fontes.feriados`), geradas da mesma lista da DDL que gera os assets, de modo que o SQL escrito para o MySQL roda sobre o parquet quase sem mexer. As views são cruas: toda linha aparece, inclusive as marcadas como excluídas, e filtrar é do consulente, porque esconder a linha morta seria a silver começando cedo demais. O DuckDB fala com o S3 pela extensão `httpfs`, configurada a partir do recurso `Lake` e pré-instalada na imagem do Dagster (nota de 23/09 no [ADR-0005](adr/0005-duckdb-parquet-medalhao.md), com as medições que decidiram). A conservação de linhas entre a réplica e a bronze passou a ser medida: o job `conferir_bronze` compara as 76 tabelas (linhas vivas na bronze contra a contagem na réplica) e falha nominando as divergentes, e a régua ganhou o check C-06 com a mesma medida, calculada pelo aceite (`--aceite --replica`, agora 166 de 166). Provado de dentro do container: 76 tabelas conferidas, 0 divergentes (card 6.1). **A auditoria de qualidade foi feita, às cegas**: dez notebooks em [`notebooks/auditoria`](../notebooks/auditoria/README.md), um por domínio e um de fechamento, executados e versionados com as saídas, tendo como únicas referências a DDL e o `docs/02` (um teste reprova célula que consulte o gerador ou a régua). Cada domínio passou por tipo contra a DDL, completude, unicidade por chave e por conteúdo, domínios declarados, texto, cronologia, integridade referencial e regras do negócio, com as declarações do cliente testadas como hipóteses. Resultado: 61 achados (4 altos, 17 médios, 19 baixos, 21 registros de severidade nenhuma), as seis declarações do `docs/02` §11 confirmadas com número, doze achados médios que ninguém declarou, e dois defeitos do próprio pipeline encontrados e corrigidos (`TINYINT` copiado como booleano; `TIME` copiado como duração): a régua de conservação conta linhas e nenhuma contagem acusa tipo errado, por isso a checagem de tipo entrou em toda auditoria. Os achados estão em [`dados/auditoria`](../dados/auditoria/), a entrada do catálogo (card 6.2). O catálogo foi escrito como código e aprovado entrada a entrada antes de qualquer transformação ([`docs/13`](13_catalogo_de_achados.md), card 6.3). **A silver existe** (card 6.4): as 76 tabelas, cada uma com o grão, as colunas e os valores da bronze, mais as colunas das 34 regras aprovadas (marcas `q_`, colunas derivadas, o nome conformado); as seis entradas restantes são da gold ou só documentação. Cada tabela é gravada em `silver/_em_conferencia/`, conferida em quatro provas (linhas por ano iguais às da bronze, valores e tipos originais intactos em `EXCEPT ALL` nos dois sentidos, colunas na ordem, marca só em linha viva) e só então publicada; a reprovada nunca substitui a publicada. A prestação de contas roda cada regra na data da auditoria e exige o número que o catálogo aprovou: 34 de 34. Provado de dentro do container pelo job `construir_silver` (77 etapas sem falha, entre 3 e 5 minutos nas rodadas de 24/09); pseudonimização e retenção a seguir |
