...
| 6.3 | o catálogo de achados como código: 40 entradas com a redação para quem decide e a para quem implementa, cinco verbos de tratamento, `docs/13` gerado dos registros e aprovado pelo Tiago antes de transformar | DQF cap. 6, *Fixing Data Quality Issues at Scale*, seção *Root Cause Analysis* · acervo; DQF cap. 8, *Treating Your "Data" Like a Product* · acervo | achado sem causa não vira regra; e a regra aprovada por quem conhece o negócio é a versão de "dono do dado" do capítulo 8 |
| 6.4 | a silver: a bronze linha a linha mais as colunas das 34 regras aprovadas (marcas `q_`, colunas derivadas, o nome conformado), gravada numa área de conferência, conferida em quatro provas e só então publicada; a prestação de contas roda cada regra na data da auditoria e exige o número aprovado | DEDP cap. 9, *Quality Enforcement*, padrões *Audit-Write-Audit-Publish* e *Constraints Enforcer* · acervo; DEDP cap. 5, *Data Decoration*, padrões *Wrapper* e *Metadata Decorator* · acervo; DEDP cap. 4, *Overwriting*, padrão *Data Overwrite* · acervo; DQF cap. 3, *Ensuring Data Quality During Transformation* · acervo; FoDE cap. 8, seção *Transformations* · acervo | no DEDP, o *Audit-Write-Audit-Publish* é a área `silver/_em_conferencia/` (gravar, auditar, só então publicar), e a marca `q_` é decoração: o dado original intacto, com a informação de qualidade ao lado; o *Data Overwrite* explica por que regravar o ano inteiro é idempotente; no DQF, a transformação testada contra o resultado esperado; no FoDE, a transformação em lote |
